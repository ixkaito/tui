 ...
 */
function tui_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'tui_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'tui' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => tui_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'tui' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tui' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'tui' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tui' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'tui' );
}
add_action( 'customize_register', 'tui_customize_register', 11 );

/**
 * Register color schemes for TUI.
 *
 * Can be filtered with {@see 'tui_color_schemes'}.
 *
 ...
 */
function tui_get_color_schemes() {
	return apply_filters( 'tui_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'tui' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'tui_get_color_scheme' ) ) :
/**
 ...
 */
function tui_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = tui_get_color_schemes();
...
endif; // tui_get_color_scheme

if ( ! function_exists( 'tui_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for TUI.
 *
 * @since TUI 1.0
 *
 * @return array Array of color schemes.
 */
function tui_get_color_scheme_choices() {
	$color_schemes                = tui_get_color_schemes();
...
endif; // tui_get_color_scheme_choices

if ( ! function_exists( 'tui_sanitize_color_scheme' ) ) :
/**
 ...
 */
function tui_sanitize_color_scheme( $value ) {
	$color_schemes = tui_get_color_scheme_choices();
...
endif; // tui_sanitize_color_scheme
...
function tui_color_scheme_css() {
	...
	$color_textcolor_rgb         = tui_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = tui_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = tui_get_color_scheme_css( $colors );

	wp_add_inline_style( 'tui-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'tui_color_scheme_css' );
 ...
 * @since TUI 1.0
 */
function tui_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', tui_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'tui_customize_control_js' );
 ...
 * @since TUI 1.0
 */
function tui_customize_preview_js() {
	wp_enqueue_script( 'tui-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'tui_customize_preview_js' );
 ...
 */
function tui_get_color_scheme_css( $colors ) {
 ...
 * @since TUI 1.0
 */
function tui_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-tui-color-scheme">
		<?php echo tui_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'tui_color_scheme_css_template' );
